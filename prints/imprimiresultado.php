...
require_once "../modelos/imprimirfinalModelo.php";
...
$resultadopdf = new imprimirfinalModelo();
$data=$resultadopdf->imprimir_resultadofinal_modelo($codigob);
$rows=$data->fetch();

//print_r($cuadrante);
//die();

$codLamina=$rows['redosCodiLamina'];
...
$reEspecimenInsatisfactorio=$rows['redosEspecimenInsatisfactorio'];
$reEspecimenRechazado=$rows['redosEspecimenRechazado'];
$reEspecimenProcesadoRechazado=$rows['redosEspecimenProcesadoRechazado'];
$glandular=$rows['Glandular'];
$cgNombre=$rows['cgNombre'];
$reCelulasGlandulares=$rows['redosCelulasGlandulares'];
$clageneral=$rows['Clasifiacion'];
$reClagAnormalidades=$rows['redosClagAnormalidades'];
$reClagClandularEscamoso=$rows['redosClagClandularEscamoso'];
$reNeoplasiasMalignas=$rows['redosNeoplasiasMalignas'];
$reCarcinoma=$rows['redosCarcinoma'];
$escamosa=$rows['Escamosa'];
$benignos=$rows['Benignas'];
$reBenignoInfeccion=$rows['redosBenignoInfeccion'];
$reInflamacionSiNo=$rows['redosInflamacionSiNo'];
$reOtrosBenigno=$rows['redosOtrosBenigno'];
$hormonal=$rows['Hormonal'];
$rePatronHormonal=$rows['redosPatronHormonal'];
$reValoracionHormonal=$rows['redosValoracionHormonal'];
$reConclusiones=$rows['redosConclusiones'];
$reObservacion=$rows['redosObservacion'];
$reFechaResultado=$rows['redosFechaResultado'];
$reNuevaMuestraSiNo=$rows['redosNuevaMuestraSiNo'];
$drNombre=$rows['drNombre'];
$drApellido=$rows['drApellido']; //agregar
//$teNombre=$rows['teNombre'];
$nombreR=$rows['NombreR'];
$apellidoR=$rows['ApellidoR'];
$colegiatura=$rows['Colegiatura'];
$firmat=$rows['tecnicofirma'];
$firmadr=$rows['drfirma'];
$colegiaturadr=$rows['drColegiatura'];
//$firmadr=$rows['tefotofirma'];
...
$pdf->SetFont('Times','',10);
$pdf->Cell(63,10, "$responsable $resApellido" ,1 ,0, 'L');
if($colNormal=="Normal"){
    $pdf->Cell(12,10, "Normal" ,1 ,0, 'C');
    $pdf->Cell(12,10, "X" ,1 ,0, 'C');
    $pdf->Cell(14,10, "Anormal " ,1 ,0, 'C');
...
$pdf->SetFont('Times','B',10);
$pdf->Cell(128,10,utf8_decode("DATOS DE LOS PROFESIONALES RESPONSABLES"),1 ,0, 'C');
$pdf->Ln(10);
$pdf->SetFont('Times','',8);
$pdf->Cell(62,10, utf8_decode(" IREN CENTRO") ,1, 0,'L');
$pdf->Cell(64,10,utf8_decode("PRIMERA LECTURA"),1 ,0, 'L');
$pdf->Cell(64,10,utf8_decode("SEGUNDA LECTURA"),1 ,0, 'L');
/*$pdf->Cell(64,10,utf8_decode("$teNombre"),1 ,0, 'L');*/
...
$pdf->Cell(64,10,utf8_decode("$drNombre $drApellido"),1 ,0, 'L');
$pdf->Ln(12);
$pdf->Cell(62,30, utf8_decode("") ,0, 0,'L');
$pdf->Cell(64,30, $pdf->Image("$firmat",$pdf->GetX(), $pdf->GetY(),50),0, 'C');
$pdf->Cell(64,30, $pdf->Image("$firmadr",$pdf->GetX(), $pdf->GetY(),50),0, 'C');
