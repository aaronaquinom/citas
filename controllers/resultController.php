...
class resultController extends resultModel{
    public function agregar_resultadofinal_controlador()
    {
                ...
                if($optionsNuevam=="Si"){
                    ...
                    else
                    {
                        $cmbEspecimen=mainModel::limpiar_cadena($_POST['cmbEspecimen']); 
                                        ...
                                        $codigoL=mainModel::limpiar_cadena($_POST['cod-lectura']);
                                        $codigoR=mainModel::decryption ($codigoR);
                                        $codigoE=mainModel::decryption ($codigoE);
                                        $codigoL=mainModel::decryption ($codigoL);
                            
                                        $querye=mainModel::ejecutar_consulta_simple("SELECT idEmployee FROM employees WHERE usCode ='$codigoE'");
                                        $c1=$querye->rowCount();
                                            if($c1>=1)
                                            {
                                                $consulta0=mainModel::ejecutar_consulta_simple("SELECT idReceptionSample FROM firstreadings  WHERE idFirstreading='$codigoR'");
                                                $recepcionmuestra=$consulta0->fetch();
                                                $idReceptionSample=$recepcionmuestra['idReceptionSample'];


                                                $consulta1=mainModel::ejecutar_consulta_simple("SELECT idgynecologicalhistory FROM receptionsamples WHERE idReceptionSample='$idReceptionSample'");
                                                $muestra=$consulta1->fetch();
                                                $idgynecologicalhistory=$muestra['idgynecologicalhistory'];
                                                ...
                                                $guardarresultado=resultModel::agregar_resultadofinal_modelo($datos); 
                                                $modificarmuestra=resultModel::modificar_muestra_modelo($idgynecologicalhistory);//modificar muestra 
                                                //$modificarlectura=resultModel::modificar_lectura_modelo($codigoL);//modificar lectura                       
                                                    $alerta=[ 
                                                    "Alerta"=>"limpiar",
                                                    "Titulo"=>"Lectura registrada",
                                                    "Texto"=>"Resultado final guardado",
                                                    "Tipo"=>"success"
                                                    ];   
                                            }else
                                            {
    ...
    public function datos_resultado_controlador($tipo, $codigo){ 
        $codigo=mainModel::decryption ($codigo);
        $tipo=mainModel::limpiar_cadena($tipo);

        return resultModel::datos_resultado_modelo($tipo, $codigo);
    }
     //controlador para paginar recepcion de muestras
    
     public function paginador_resultado_controlador($pagina, $registros, $privilegio, $url, $tipoempleado, $empleado, $busqueda)
     {
         ...
         if(isset($busqueda) && $busqueda!=""){
             $consulta="SELECT SQL_CALC_FOUND_ROWS  idResult, hgRecepcionYesNo, frCodeLamina, reDateResult, medico.emLastName as Medico, tecnologo.emName as Lectura, paDNI,
           ...
           ORDER BY idResult DESC
            ...
            switch($tipoempleado){
               ...
               ORDER BY idResult DESC
                ...
                case "Muestreador":
                    $consulta="SELECT SQL_CALC_FOUND_ROWS  idResult, hgRecepcionYesNo, frCodeLamina, reDateResult, medico.emLastName as Medico, tecnologo.emName as Lectura, paDNI,
                    paName, paLastName, esName, tabla1.emName as Muestra, hgDateSample, hgStateSample, usuario.uscode as codigou
                  FROM results inner join firstreadings on results.idFirstreading=firstreadings.idFirstreading
                  inner join employees as medico on results.idEmployee=medico.idEmployee
                  inner join receptionsamples on firstreadings.idReceptionSample=receptionsamples.idReceptionSample 
                  inner join employees as tecnologo on firstreadings.idEmployee=tecnologo.idEmployee
                  ...
                  inner join patients on patients.paCode=gynecologicalhistorys.paCode
                  inner join employees as tabla1 on gynecologicalhistorys.idEmployee=tabla1.idEmployee
                  inner join users as usuario on tabla1.usCode=usuario.usCode
                  inner join establishments on patients.idEstablishment=establishments.idEstablishment
                  inner join employees as tabla2 on receptionsamples.idEmployee=tabla2.idEmployee
                  WHERE usuario.uscode LIKE '%$empleado%'
            ...
            <thead>
                <tr>
                   ...
                   <!-- <th class="text-center">RESPONSABLE LECTURA</th>-->
                    ...
                    <th class="text-center">DATOS RESULTADO</th>
            ...
            foreach($datos as $rows){
                ...
                $tabla.='
                    <tr>
                        ...
                        $tabla.='
                        <td>
                        <a href="'.SERVERURL.'sample-print/'.mainModel::encryption($rows['idResult']).'/" class="btn btn-success btn-raised btn-xs">
                        ...
                        if($rows['hgRecepcionYesNo']=="0"){
                            ...
                            $tabla.='
                            <td>
                             <a href="'.SERVERURL.'result-print/'.mainModel::encryption($rows['idResult']).'/" class="btn btn-success btn-raised btn-xs">
                            <i class="zmdi zmdi-refresh"></i>
                            </a>
                            </td>';
                        }
                        ...
                        $tabla.='
                        <!--<td>
                        <a href="'.SERVERURL.'sample-update/'.mainModel::encryption($rows['idResult']).'/" class="btn btn-success btn-raised btn-xs">
                            <i class="zmdi zmdi-refresh"></i>
                        </a>
                        </td>-->
                        ';
